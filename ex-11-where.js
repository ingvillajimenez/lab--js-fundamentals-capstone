...
function where(objList, obj) {
  var matchedObjList = [];
  var propertyNumbers = Object.keys(obj).length;
  for (var i = 0; i < objList.length; i++) {
    var matchNumbers = 0;
    for (var keysObjList in objList[i]) {
      for (var keysObj in obj) {
        if(objList[i][keysObjList] === obj[keysObj]){
          matchNumbers++;
        }
      }
    }
    if(matchNumbers === propertyNumbers){
      matchedObjList.push(objList[i]);
    }
  }
  // console.log(matchedObjList);
